...
- **Clearing an already-delivered notification on other devices requires client support.** As of this writing, ntfy's docs list this (`message_delete` / notification cancellation) as `Supported on:` Android and the Firefox web app only — other clients, including the iOS app, likely just keep showing the notification until dismissed by hand. Check ntfy's own docs for the current support matrix.
