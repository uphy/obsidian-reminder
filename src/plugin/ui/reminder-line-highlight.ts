...
/** Builds the single mark decoration for the range, or `Decoration.none` if the range is empty (e.g. flashing a blank line). */
function buildFlashDecoration(range: {
  from: number;
  to: number;
}): DecorationSet {
 ...
 */
const reminderLineFlashField = StateField.define<DecorationSet>({
  create() {
    return Decoration.none;
  },

  update(decos, tr) {
    const flash = tr.effects.find((e) => e.is(flashReminderLine));
    if (flash !== undefined) {
      return buildFlashDecoration(flash.value);
    }
...
/** CM6 extension: highlights a range dispatched via `flashReminderLine` until the user's next cursor move or edit. */
export function createReminderLineFlashExtension(): Extension {
  return reminderLineFlashField;
}
