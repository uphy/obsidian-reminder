...
- Official release: run `release.yml` on master with a `bump` input (`patch`/`minor`/`major`, default `patch`) instead of an explicit version — e.g. `gh workflow run release.yml` (defaults to patch) or `gh workflow run release.yml -f bump=minor`. The workflow checks that manifest.json's and package.json's versions are in sync (fails with an error otherwise), computes the next version from manifest.json via `npm version <bump>` (which also bumps package.json/package-lock.json), and updates manifest.json (version) and versions.json (appends the new version, paired with manifest.json's `minAppVersion`) → builds → commits the version bump, pushes it to a temporary `release/<version>` branch and waits for that commit's "test" check (run by `pr.yml`, which also triggers on `release/**`, and covers lint/type-check/tests/version-consistency) to pass, then pushes the same already-checked commit to master and deletes the temporary branch (master's branch protection requires a passing "test" status check per commit, which a direct push carries none of) → tags the release → creates a GitHub Release with auto-generated release notes → then (after the release job succeeds) deploys the docs site to gh-pages.
...
- Both workflows push (and `release-beta.yml` also opens its PR) using an installation token for the `uphy-ops` GitHub App instead of `GITHUB_TOKEN`, because GITHUB_TOKEN-authored pushes and PRs don't trigger other workflows, which would starve the required "test" check. This requires the `RELEASE_APP_ID` and `RELEASE_APP_PRIVATE_KEY` repository secrets.
