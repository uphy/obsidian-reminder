...
- Official release: run `release.yml` on master via `workflow_dispatch` → updates manifest.json and pushes directly to master → zips the build artifacts and uploads them to a GitHub Release.
- Beta release: run `release-beta.yml` on the equivalent of the develop branch → updates manifest-beta.json, pushes to develop, and tags it → automatically opens a PR from a branch that cherry-picks that change onto master (merging is manual) → creates a GitHub Release marked as prerelease.
- `versions.json` maps plugin versions to the minimum supported Obsidian version. When adding a new version, append it paired with `MINIMUM_OBSIDIAN_VERSION` (hardcoded to 1.0.3 in release.sh).
